...
func ExportSetList(setList SetList, outputFileName string) {
	...
	for _, track := range setList.Tracks {
		...
		fmt.Fprintf(file, "%.6f\t%.6f\t%s\n", tsFloat, tsFloat, track.Performer+" - "+track.Title)
	}
}

func main() {
	inputCueFile := flag.String("input", "", "Input CUE file")
	outputFile := flag.String("output", "", "Output LBL file")
	flag.Parse()

	if *inputCueFile == "" || *outputFile == "" {
		log.Fatal("Usage: go run main.go -input <input.cue> -output <output.lbl>")
	}

	// open file
	f, err := os.Open(*inputCueFile)
	if err != nil {
		log.Fatal(err)
	}
	...
	log.Println("Exporting to LBL file: ", *outputFile)
	ExportSetList(*SetList, *outputFile)
	log.Printf("Exported %d tracks to %s\n", SetList.TrackCount, *outputFile)
